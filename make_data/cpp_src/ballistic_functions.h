...
//TODO i don't know if it impacts anything, but if a shell impacts a target on "ascent"
// it's delta time will be huge, as time_in_air will only end execution when shell goes above and
// then below target. Maybe return both time when projectile went above the target y and below to
// account for such case? again, idk if it impacts anything though.

//TODO add a rule that if projectile stops ascending but did not went above target y then it will
// return early
...
//TODO somehow determine if position is too close and high for cannon to hit.
inline std::pair<std::array<float, 3>, bool> calculate_pitch(
    ...
    auto vertical_airtime = time_in_air(y_coord_end_of_barrel, target[1], Vy, max_steps);
    if (vertical_airtime < 0) { return {{-1, -1, -1}, false};}
    ...
    if (vertical_airtime - delta_t_max_overshoot < horizontal_time_to_target) { return {{-1, -1, -1}, false};}

    auto delta_t = std::abs(horizontal_time_to_target - vertical_airtime);

    return {{(float)delta_t, tried_pitch, (float)vertical_airtime}, true};
}
...
rough_pitch_estimation(const std::array<float, 3> & cannon,
    ...
    for (int tried_pitch = 60; tried_pitch >= -30; tried_pitch--) {
        auto [items, is_successful] = calculate_pitch(cannon, target, tried_pitch, distance, initial_speed, length, max_steps, delta_t_max_overshoot);
...
inline void calculate_y_line(std::vector<std::array<std::array<float, 3>, 2>> *dataset, int32_t charges, int barrel_length,
                      ...
                      uint32_t impossible_cutoff, float delta_t_max_overshoot, float step, int y, bool count_cutoff_at_the_start) {
