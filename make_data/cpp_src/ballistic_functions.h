...
inline std::array<float, 3>
    ...
    auto delta_times2 = fine_pitch_estimation(cannon, target, distance, initial_speed, length, min_pair.second, 20, max_steps);
...
inline void calculate_y_line(std::vector<std::array<std::array<float, 3>, 2>> *dataset, int32_t charges, int barrel_length,
                      ...
                      uint32_t impossible_cutoff, float step, int y) {
    int cutoff_count = 0;
    for (float x = barrel_length; x < max_length; x += step) {
        auto res = pitch_fn({0, 0, 0}, {x, y, 0}, charges, barrel_length, max_simulation_steps);
        if (res[0] >= 0) {
        ...
        if (res[0] < 0) {cutoff_count++;} else {cutoff_count = 0;}
