...
def calculate_y_line(dataset, y, charges, starting_x, barrel_length, points_simulated, y_done, max_length,
    ...
    while x < max_length:
        ...
        if res[0] >= 0:
            ...
            if not dont_change_starting: starting_x = x
...
def make_dataset_thread(dataset, charges, length, max_height_above, max_height_below,
    ...
    if start_pos == 0:
        calculate_y_line(dataset, 0, charges, length, length, progress[0], progress[1], max_length, max_simulation_steps,
                         impossible_cutoff, delta_t_max_overshoot, step, True, amin, amax, gravity, drag, num_elements,
                         num_iterations, check_impossible)

    starting_x = length
    for y in range(start_pos+1, max_height_above, num_threads):
        starting_x = calculate_y_line(dataset, y, charges, starting_x, length, progress[0], progress[1], max_length, max_simulation_steps,
                         impossible_cutoff, delta_t_max_overshoot, step, True, amin, amax, gravity, drag, num_elements,
                         num_iterations, check_impossible)
